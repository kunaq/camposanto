...
class ModeloVPS{
	...
	static public function mdlBuscaBenef($tabla1,$tabla2,$tabla3,$datos){
    	...
    	while($key = $db->recorrer($sql)){
			$key["fch_servicio"] = ($key["fch_servicio"] != '') ?  date_format($key['fch_servicio'], 'd-m-Y H:i') : '-';
    		$datos[] = array_map('utf8_encode', $key);
		}
		return $datos;
		$db->liberar($sql);
        $db->cerrar();
	...
	static public function mdlStoreTabla($datos){
		...
		do {
			while($key = $db->recorrer($sql)){
	    		$key["fch_fecha"] = ($key["fch_fecha"] != '') ?  date_format($key['fch_fecha'], 'd-m-Y H:i') : '-';
	    		$respuesta[] = arrayMapUtf8Encode($key);
			}
