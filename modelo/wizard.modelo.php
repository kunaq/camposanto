  ...
  //   	while($key = $db->recorrer($sql)){
	 //    		$datos[] = arrayMapUtf8Encode($key);
		...
		while($key = $db->recorrer($sql)){
	    		...
	    		$arrData = array('num_contrato'=> $num_contrato, 'num_servicio'=>$num_servicio, 'cod_tipo_ctt'=>$cod_tipo_ctt, 'cod_tipo_programa'=> $cod_tipo_programa, 'num_refinanciamiento'=>$num_refinanciamiento, 'cod_localidad'=>$cod_localidad);
			}
