...
class ModeloModifCtto{
	...
	static public function mdlValUsoServ($tablaAut,$tablaEdo,$datos){
		...
		echo "SELECT COUNT(1) FROM $tablaAut INNER JOIN $tablaEdo ON $tablaEdo.cod_estado_autorizacion = $tablaAut.cod_estado_autorizacion WHERE $tablaEdo.flg_anulado = 'NO' AND $tablaAut.cod_localidad_ctt = '".$datos['ls_localidad']."' AND $tablaAut.cod_contrato = LIKE (RIGHT('0000000000'+'".$datos['ls_contrato']."',10)) AND $tablaAut.num_servicio = ".$datos['ls_servicio']." AND $tablaAut.cod_tipo_programa = '".$datos['ls_tipo_programa']."' AND $tablaAut.cod_tipo_ctt = '".$datos['ls_tipo_ctt']."'";
		$datos = arrayMapUtf8Encode($db->recorrer($sql));
		return $datos;
		$db->liberar($sql);
        $db->cerrar();
