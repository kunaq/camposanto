...
class ModeloModifCtto{
	...
	static public function mdlBuscaDatosServicio($tabla,$tabla2,$tabla3,$codCtto,$num_servicio){
		$db = new Conexion();
		$sql = $db->consulta("SELECT $tabla.*, $tabla2.*, $tabla3.dsc_entidad FROM $tabla INNER JOIN $tabla2 ON ($tabla.cod_contrato = $tabla2.cod_contrato) LEFT JOIN $tabla3 ON $tabla3.cod_entidad = $tabla.cod_convenio WHERE $tabla.cod_contrato  LIKE (RIGHT('0000000000'+'$codCtto',10)) AND $tabla.num_servicio = $num_servicio AND $tabla2.num_servicio = $num_servicio");
