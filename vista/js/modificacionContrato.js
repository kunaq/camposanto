            	...
            	$.each(respuesta,function(index,value){
                    ...
                    if (respuesta[0]['flg_ctt_integral'] == 'SI') {
                        ...
                        var fila2 = '<tr name="'+value['num_servicio']+'" style="'+color+'">'+
                            ...
                            '<td class="text-right">'+Number(value['imp_saldofinanciar']).toLocaleString('en-US',{ style: 'decimal', maximumFractionDigits : 2, minimumFractionDigits : 2 })+'<input type="hidden" id="is_principal_'+value['num_servicio']+'" value="'+value['flg_principal']+'"><input type="hidden" id="is_ds_'+value['num_servicio']+'" value="'+value['flg_derecho_sepultura']+'"><input type="hidden" id="flg_afecto_igv_'+value['num_servicio']+'" value="'+value['flg_afecto_igv']+'"><input type="hidden" id="imp_cuoi_'+value['num_servicio']+'" value="'+value['imp_cuoi']+'"></td>'+
                        '</tr>';
            ...
            $.each(respuesta,function(index,value){
                var registro = [value['cod_tipo_documento'],value['dsc_documento'],value['dsc_apellidopaterno'],value['dsc_apellidomaterno'],value['dsc_nombre'],value['fch_nacimiento'],value['fch_deceso'],value['cod_religion'],value['cod_estado_civil'],value['cod_sexo'],value['cod_parentesco'],value['cod_lugar_deceso'],value['cod_motivo_deceso'],value['num_peso'],value['num_talla'],value['flg_autopsia'],value['num_item'],value['num_servicio'],value['fch_entierro'],value['num_nivel'],'NO'];
                 var muestra = '<tr onclick="verDetalles(event)" id="'+value['num_item']+'">'+
                        '<td class="'+value['num_item']+'">'+value['dsc_apellidopaterno']+' '+value['dsc_apellidomaterno']+', '+value['dsc_nombre']+
    ...
    container.querySelectorAll('tr').forEach(function (i){ 
        ...
        if(i == '' || flg_nuevo_benef == 'SI'){
            $.ajax({
              ...
              dataType: 'text',
              data: {'accion' : 'guardaBeneficiario', 'ls_localidad' : localidad, 'ls_num_contrato_new' : cod_contrato, 'li_linea_benef' : numItem, 'ls_num_servicio_new' : numServ, 'ls_ape_paterno_benef' : apellPaterno, 'ls_ape_materno_benef' : apellMaterno, 'ls_nombre_benef' : nombre, 'ls_tipo_doc_benef' : tipoDoc, 'ls_num_doc_benef' : numDoc, 'ldt_nacimiento' : fechNac, 'fch_entierro' : fchEnt, 'ldt_deceso' : fechDec, 'ls_religion' : religion, 'ls_lugar_deceso' : lugar, 'ls_motivo_deceso' : motivo, 'ls_flg_autopsia' : autopsia, 'lde_peso' : peso, 'num_nivel' : nivel, 'lde_talla' : talla, 'ls_parentesco' : parentesco, 'ls_estado_civil' : edoCivil, 'ls_sexo' : sexo, 'ls_tipo_ctt_new' : ls_tipo_ctt_new, 'ls_tipo_programa_new' : ls_tipo_programa_new},
              success : function(respuesta){
                if(respuesta == 1){
